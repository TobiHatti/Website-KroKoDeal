...
function FileButton($name, $id, $multiple=false, $onchange="", $onclick="",$style="",$required=false)
{
    // DESCRIPTION:
    ...
    return  '
        <input type="file" name="'.$name.'[]" id="'.$id.'" class="inputfile" data-multiple-caption="{count} Dateien" '.(($multiple) ? 'multiple' : '').' '.(($onchange!="") ? ('onchange="'.$onchange.'"') : '').' hidden '.($required ? 'required' : '').'/>
